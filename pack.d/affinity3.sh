...
# The installed setup script prepares a per-user Wine prefix on first launch.
...
dotnet48_registered()
{
    for key in \
        ...
        'HKLM\Software\Wow6432Node\Microsoft\NET Framework Setup\NDP\v4\Full' ; do
        registry="$("$WINE" reg query "$key" 2>/dev/null)" || continue
        install="$(printf '%s\n' "$registry" | awk '$1 == "Install" && $2 == "REG_DWORD" { print $3; exit }')"
        case "$install" in
            0x1|0x00000001) ;;
            *) continue ;;
        esac

        version="$(printf '%s\n' "$registry" | awk '$1 == "Version" && $2 == "REG_SZ" { print $3; exit }')"
        case "$version" in
            4.8*) return 0 ;;
        esac

        release="$(printf '%s\n' "$registry" | awk '$1 == "Release" && $2 == "REG_DWORD" { print $3; exit }')"
        case "$release" in
            0x*) release="${release#0x}" ;;
            *) continue ;;
        esac
        case "$release" in
            ''|*[!0-9a-fA-F]*) continue ;;
        esac
        [ "$((0x$release))" -ge "$((0x80e18))" ] && return 0
    done
    return 1
}
...
# Install and validate the bundled Wine runtime.
...
"$WINE" --version >/dev/null 2>&1 || fatal 'bundled Wine cannot run'
...
# Prepare the prefix and its Windows dependencies.
...
dotnet48_registered || say 'Warning: .NET Framework 4.8 was not found in the Wine registry; the Affinity installer may fail.'

# Install Vulkan D3D12 support.
...
# Run the packaged Affinity installer when its build changes.
...
# Apply the settings patch and install per-user application settings.
...
# Install the optional launch hook and Wine compatibility plugin.
...
# The installed launcher reuses the prepared prefix and forwards all arguments.
...
export WINEDEBUG="-all,fixme-all"
export WINEDLLOVERRIDES="winemenubuilder.exe=d"
# DXVK reads this host path when its D3D9 implementation is in use.
export DXVK_CONFIG_FILE="${DXVK_CONFIG_FILE:-$PRODUCTDIR/dxvk.conf}"
exec "$RUNTIME/bin/wine" "$RUNFILE" "$@"
EOF

# DXVK reads this only when its D3D9 implementation is active.
cat <<'EOF' >"$PRODUCTDIR/dxvk.conf"
# Keep Affinity context menus and flyouts from rendering black with DXVK D3D9.
